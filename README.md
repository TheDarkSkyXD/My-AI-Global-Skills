...
This repo includes an `AGENTS.md` file with global behavioral rules for AI coding agents. It enforces:

- **Concise responses** — no fluff unless asked
- **Planning mode** — always ask clarifying questions, never assume design/tech stack/features, use sub-agents for research and plan review
- **Change/edit mode** — state assumptions, surface tradeoffs, delegate to sub-agents, coordinate in parallel, run lint/type-check/build after changes
- **Testing** — always verify changes with available testing tools; ask before skipping
- **UI design** — follow the project's `DESIGN.md` system (auto-created for frontend projects if missing)

## Claude Code Config (CLAUDE.md)

The `CLAUDE.md` file contains a single `@AGENTS.md` directive, which tells Claude Code to load all rules from `AGENTS.md`. This keeps configuration DRY — one source of truth for agent behavior that works across Claude Code, Codex, Cursor, and any agent that reads `AGENTS.md`.

Copy both files to your project root to apply these rules globally.

## Installation

Clone this repo and copy (or symlink) the skills you want into your project:
